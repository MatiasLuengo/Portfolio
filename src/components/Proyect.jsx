export const Proyect = ({ img, imgAlt, title, description, url, utilities, orderImg }) => {
    return (
        <a href={url} target='_blank' className="w-ful mb-14 sm:w-10/12 m-auto  rounded-md grid grid-flow-row auto-rows-max lg:grid-rows-none lg:grid-cols-2 bg-white hover:scale-105 duration-150 shadow-[4.0px_8.0px_8.0px_rgba(0,0,0,0.38)]">
            ...
            <div className='p-2 flex flex-col justify-between'>
                ...
                <div className='border-t-2 flex flex-wrap items-center justify-center gap-3 p-2'>
                    {utilities.map((utility, index) => (
                        <p key={index} className={`rounded-2xl ${utility.color} text-white py-1 px-2 flex items-center gap-1 shadow-lg`}>
